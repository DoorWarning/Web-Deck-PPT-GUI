...
function Render({ block }: BlockRenderProps) {
  ...
  return (
    <div className={`playground ${p.showEditor ? 'with-editor' : ''}`}>
      ...
      <iframe className="pg-preview" title={block.id} sandbox="allow-scripts" srcDoc={srcdoc} />
    </div>
  );
}
...
export const playgroundBlock: BlockDef = { type: 'playground', label: '플레이그라운드', icon: '⚡', Render, Edit };
